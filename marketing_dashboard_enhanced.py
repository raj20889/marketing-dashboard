...
@st.cache_data
def load_data():
    marketing = pd.read_csv("/mnt/data/marketing_cleaned_raw.csv", parse_dates=['date'])
    daily_totals = pd.read_csv("/mnt/data/daily_totals.csv", parse_dates=['date'])
    daily_merged = pd.read_csv("/mnt/data/daily_merged_business_marketing.csv", parse_dates=['date'])
...
mkt = marketing[(marketing['date']>=start_date)&(marketing['date']<=end_date)&(marketing['channel'].isin(selected_channels))].copy()
...
if page == "Overview":
    ...
    st.markdown("This section estimates how ad spend leads or lags business orders (cross-correlation). It computes channel-level cross-correlation between daily spend and # of orders.")
    # prepare daily series
    orders = dm[['date']].copy()
    # find plausible orders column
    ...
    st.markdown("We attribute daily new customers to channels proportionally based on that day's attributed_revenue by channel. This is a pragmatic, transparent approach for the assignment.")
    # ensure new_customers exists
    if 'new_customers' not in dm.columns:
        st.error('No new_customers column found in business data.')
    else:
        # compute attributed_revenue by date+channel
        ...
        # merge with new_customers per date
    ...
    if st.button('Download channel-level KPIs CSV'):
        df = mkt.groupby('channel').agg(impressions=('impressions','sum'), clicks=('clicks','sum'), spend=('spend','sum'), attributed_revenue=('attributed_revenue','sum')).reset_index()
    ...
    if st.button('Download summary PPTX'):
        with open('/mnt/data/marketing_summary_slides.pptx','rb') as f:
            st.download_button('Download Slides', f.read(), file_name='marketing_summary_slides.pptx')
    st.markdown('Dockerfile and requirements.txt included for easy deployment.')
